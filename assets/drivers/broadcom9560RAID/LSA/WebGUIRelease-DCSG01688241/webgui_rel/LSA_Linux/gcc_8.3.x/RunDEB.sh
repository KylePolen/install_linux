...
if [ $utils_pkg = 0 ]; then
	echo "Installing  LSA_lib_utils-1.17-00"
        dpkg -i LSA_lib_utils-1.17-1_amd64.deb
else
...
service_restart ()
{
	...
	test_crypt_lib=`ldconfig -p | grep libcrypt.so.1`
        CRYPT_LIB="libcrypt.so.2"
        test_crypt_lib_count=`echo $test_crypt_lib | wc -m`
        if [ $test_crypt_lib_count -gt 1 ] ; then
                target=`echo $test_crypt_lib | awk '{print $4}' | head -1`
                ln -s $target /opt/lsi/LSIStorageAuthority/bin/$CRYPT_LIB 2>|/dev/null
                ln -s $target /opt/lsi/LSIStorageAuthority/server/$CRYPT_LIB 2>|/dev/null
        fi
