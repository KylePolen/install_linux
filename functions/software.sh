...
#Oracle VirtualBox
_soft_virtualbox() {
	if grep -q '"name":"VirtualBox"' ~/install/orderdata/orderdata; then
...
#Google Chrome
_soft_chrome() {
	if grep -q '"name":"Chrome"' ~/install/orderdata/orderdata; then
...
#Adobe Reader
_soft_reader() {
	if grep -q '"name":"Acrobat Reader"' ~/install/orderdata/orderdata; then
...
#VLC
_soft_vlc() {
	if grep -q '"name":"VLC"' ~/install/orderdata/orderdata; then
