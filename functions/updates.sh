...
_update_desktop() {
	if [ $ostype == "Desktop" ]; then
		...
		sudo DEBIAN_FRONTEND=nointeractive apt install mdadm net-tools pv finger xfsprogs dkms build-essential apt-utils ssh snap shfmt -y
...
_update_server() {
	if [ $ostype == "Server" ]; then
		...
		sudo apt update
		sudo DEBIAN_FRONTEND=nointeractive apt dist-upgrade -y
		sudo DEBIAN_FRONTEND=nointeractive apt install build-essential bolt finger pv smartmontools p7zip-full smbclient network-manager ssh dkms samba cups mdadm net-tools -y
		if [ $osversion == "20.04" ]; then
